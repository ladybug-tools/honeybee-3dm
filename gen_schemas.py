...
from honeybee_3dm.config import Config
...
if args.version:
    ...
    VERSION = '.'.join(get_distribution('honeybee-3dm').version.split('.')[:3])
...
info = {
    ...
    "license": {
        "name": "AGPL-3.0",
        "url": "https://github.com/ladybug-tools/honeybee-3dm/blob/master/LICENSE"
    }
}

with open(os.path.join(folder, 'config-openapi.json'), 'w') as out_file:
    json.dump(
        get_openapi(
            base_object=[Config], title='Honeybee-3dm Schema',
            description='Schema documentation for writing a config file.',
            version=VERSION
        ),
        out_file,
        indent=2
...
with open(os.path.join(folder, 'config-schema.json'), 'w') as out_file:
    out_file.write(Config.schema_json())
